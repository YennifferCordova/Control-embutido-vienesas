...
def cargar_datos():
 ...
 # Carta U: usar CantidadTotal si existe, si no dejar NaN 
...
def carta_xbarra(df, maquina, campo, titulo_extra, meta=None):
 ...
 if meta is not None:
 agregar_linea(fig, meta, "Meta", "#f39c12", "longdash")
...
def carta_p(df, maquina, n=10):
 ...
 marker=dict(size=6,
 symbol=["x" if v > UCL else "circle" for v in d["p"]])))
...
# CARTA U — CORREGIDA 
# UCL/LCL varían por fila porque cada turno puede tener distinta CantidadTotal
def carta_u(df_stick, maquina=None):
 ...
 # Requiere CantidadTotal real; si no existe o todos son NaN, no graficar
 if "U" not in df.columns or df["U"].isna().all():
 return None, False

 df = df.dropna(subset=["U", "CantidadTotal"])
 if df.empty:
 return None, False

 u_bar = df["TotalRotos"].sum() / df["CantidadTotal"].sum() # estimador global correcto
 ...
 # Límites variables por fila (líneas de paso)
 fechas_ucl, fechas_lcl = [], []
 vals_ucl, vals_lcl = [], []
 for _, row in df.iterrows():
 ni = row["CantidadTotal"]
 ucl = u_bar + 3 * np.sqrt(u_bar / ni)
 lcl = max(0.0, u_bar - 3 * np.sqrt(u_bar / ni))
 fechas_ucl.append(row["Fecha"]); vals_ucl.append(ucl)
 fechas_lcl.append(row["Fecha"]); vals_lcl.append(lcl)

 fig.add_trace(go.Scatter(x=fechas_ucl, y=vals_ucl, mode="lines",
 name="UCL", line=dict(color="#e24b4a", dash="dash", width=1.5), showlegend=True))
 fig.add_trace(go.Scatter(x=fechas_lcl, y=vals_lcl, mode="lines",
 name="LCL", line=dict(color="#e24b4a", dash="dash", width=1.5), showlegend=False))

 agregar_linea(fig, u_bar, "ū", "#185fa5", "dot")
 label = maquina if maquina else "Ambas máquinas"
 return estilo_base(fig, f"Carta U — Stick rotos por unidad | {label}", "Rotos / sticks producidos"), True
...
def grafico_lineas_turno(df_stick, columnas_tripa):
 ...
 d = df[df["Turno"] == turno]
 if d.empty: continue
 fig.add_trace(go.Scatter(x=d["Fecha"], y=d["TotalFiltrado"], name=f"Turno {turno}",
 mode="lines+markers", line=dict(color=color, width=2), marker=dict(size=6)))
 fig.update_layout(height=320, title="Stick rotos por turno en el tiempo",
...
def grafico_total_por_turno(df_stick, columnas_tripa):
 ...
 if df.empty: return None
 df["TotalFiltrado"] = df[columnas_tripa].sum(axis=1) if columnas_tripa else df["TotalRotos"]
 resumen = df.groupby("Turno")["TotalFiltrado"].sum().reset_index().sort_values("Turno")
 fig = go.Figure()
 fig.add_trace(go.Bar(x=resumen["Turno"], y=resumen["TotalFiltrado"],
 marker_color=[COLORES_TURNO.get(t, "#888") for t in resumen["Turno"]],
 text=resumen["TotalFiltrado"], textposition="outside"))
 fig.update_layout(height=320, title="Total stick rotos por turno",
...
def grafico_sticks_apilado(df_stick, maquina=None):
 ...
 fig.add_trace(go.Scatter(x=df["Fecha"], y=df[campo], name=nombre,
 mode="lines+markers", line=dict(color=color, width=2), marker=dict(size=6)))
 fig.update_layout(height=320, title="Comparativo stick rotos por tipo de tripa",
 xaxis_title="Fecha", yaxis_title="Cantidad",
 margin=dict(l=10, r=10, t=50, b=10), hovermode="x unified",
 legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1))
 return fig
...
tiene_cantidad = sticks is not None and "CantidadTotal" in sticks.columns and not sticks["CantidadTotal"].isna().all()

st.success(
 f"{len(muestreo)} registros de muestreo | "
 f"{len(sticks) if sticks is not None else 0} registros de stick rotos"
 + (" | Carta U activa (CantidadTotal detectada)" if tiene_cantidad
 else " | ℹ Carta U inactiva — agrega columna CantidadTotal en StickRotos para activarla")
)
...
with tab5:
 ...
 st.info(
 "ℹ La carta U está inactiva porque no se encontró la columna **CantidadTotal** "
 "en la hoja StickRotos del Excel. Agrégala con el total de sticks producidos "
 "por turno para activarla."
 )
 ...
 with c2:
 if tiene_cantidad:
 fig_u, ok = carta_u(df_stick_f, maq)
 if ok and fig_u:
 st.plotly_chart(fig_u, use_container_width=True)
 else:
 st.info(f"Sin datos suficientes de CantidadTotal para {maq}")
 else:
 st.empty() # columna vacía si no hay CantidadTotal

 st.markdown("**Comparativo stick rotos por tipo de tripa**")
 turno_bar_sel = st.multiselect("Filtrar gráfico por turno", turnos_disp,
 default=turnos_disp, key="turno_bar")
